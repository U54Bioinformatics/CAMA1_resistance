...
featureplot_umap_png <- function(cells, features, filename, figdir, size=4, width=1200, height=1200){
    ...
    font_size = 46
    FeaturePlot(cells, features = features, reduction='umap', ncol = 2) + theme(plot.title=element_text(size=font_size,face="bold"), axis.text=element_text(size=font_size, face="bold"), axis.title=element_text(size=font_size,face="bold"), legend.text=element_text(size=font_size, face="bold"), legend.title=element_text(size=font_size, face="bold")) + theme(panel.grid.major = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) + guides(shape = guide_legend(override.aes = list(size = 28)))
    #dev.off()
}
...
featureplot_tsne_png <- function(cells, features, filename, figdir, size=4, width=1200, height=1200){
    ...
    font_size = 46
    FeaturePlot(cells, features = features, reduction='tsne', ncol = 2) + theme(plot.title=element_text(size=font_size,face="bold"), axis.text=element_text(size=font_size, face="bold"), axis.title=element_text(size=font_size,face="bold"), legend.text=element_text(size=font_size, face="bold"), legend.title=element_text(size=font_size, face="bold")) + theme(panel.grid.major = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) + guides(shape = guide_legend(override.aes = list(size = 28)))
    #dev.off()
}
...
doheatmap_png <- function(cells, features, filename, figdir, width=1200, height=1200){
    ...
    png(filename, width = width, height = height)
    font_size = 26
    DoHeatmap(pbmc, features = features) + theme(plot.title=element_text(size=font_size,face="bold"), axis.text=element_text(size=font_size-10, face="bold"), axis.title=element_text(size=font_size,face="bold"), legend.text=element_text(size=font_size, face="bold"), legend.title=element_text(size=font_size, face="bold")) + theme(panel.grid.major = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) + guides(shape = guide_legend(override.aes = list(size = 8)))
    #dev.off()
}
...
figdir=paste(sample, platform, "seurat_filtered_png", sep="_")
...
dimplot_png(pbmc, 'umap', fig_tsne, figdir, "RNA_snn_res.0.5")
fig_tsne = paste(sample, platform, 'umap', "RNA_snn_res.0.8", sep="_")
dimplot_png(pbmc, 'umap', fig_tsne, figdir, "RNA_snn_res.0.8")
fig_tsne = paste(sample, platform, 'umap', "RNA_snn_res.1.2", sep="_")
dimplot_png(pbmc, 'umap', fig_tsne, figdir, "RNA_snn_res.1.2")
fig_tsne = paste(sample, platform, 'umap', "RNA_snn_res.2", sep="_")
dimplot_png(pbmc, 'umap', fig_tsne, figdir, "RNA_snn_res.2")
...
dimplot_png(pbmc, 'tsne', fig_tsne, figdir, "RNA_snn_res.0.5")
fig_tsne = paste(sample, platform, 'tsne', "RNA_snn_res.0.8", sep="_")
dimplot_png(pbmc, 'tsne', fig_tsne, figdir, "RNA_snn_res.0.8")
fig_tsne = paste(sample, platform, 'tsne', "RNA_snn_res.1.2", sep="_")
dimplot_png(pbmc, 'tsne', fig_tsne, figdir, "RNA_snn_res.1.2")
fig_tsne = paste(sample, platform, 'tsne', "RNA_snn_res.2", sep="_")
dimplot_png(pbmc, 'tsne', fig_tsne, figdir, "RNA_snn_res.2")
...
#count and features
fig_epithelial_feature = paste(prefix, "nCount_RNA", sep="_")
featureplot_umap_png(pbmc, c("nCount_RNA"), fig_epithelial_feature, figdir)
fig_epithelial_feature = paste(prefix, "nFeature_RNA", sep="_")
featureplot_tsne_png(pbmc, c("nFeature_RNA"), fig_epithelial_feature, figdir)
...
#1.2
Idents(object = pbmc) <- 'RNA_snn_res.1.2'
cells.markers <- FindAllMarkers(object = pbmc, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
#output txt file
genes.diff.file <- paste(sample, platform, "top10_genes_RNA_snn_res.1.2.gene_diff.txt", sep="_")
genes.id   <- data.table(gene = rownames(cells.markers))
genes.diff <- cbind(genes.id, cells.markers)
write.table(genes.diff, genes.diff.file, sep="\t", quote=FALSE, row.names=FALSE)
top10 <- cells.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
...
#DoHeatmap(object = pbmc, features = top10$gene)
#png
fig_markers = paste(sample, platform, "top10_genes_RNA_snn_res.1.2", sep="_")
doheatmap_png(pbmc, top10$gene, fig_markers, figdir)
dev.off()

#2
#Idents(object = pbmc) <- 'RNA_snn_res.2'
#cells.markers <- FindAllMarkers(object = pbmc, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
#output txt file
#genes.diff.file <- paste(sample, platform, "top10_genes_RNA_snn_res.2.gene_diff.txt", sep="_")
#genes.id   <- data.table(gene = rownames(cells.markers))
#genes.diff <- cbind(genes.id, cells.markers)
#write.table(genes.diff, genes.diff.file, sep="\t", quote=FALSE, row.names=FALSE)
#top10 <- cells.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
...
#DoHeatmap(object = pbmc, features = top10$gene)
#png
#fig_markers = paste(sample, platform, "top10_genes_RNA_snn_res.2", sep="_")
#doheatmap_png(pbmc, top10$gene, fig_markers, figdir)
#dev.off()
}#end of if for marker gene differential expression
