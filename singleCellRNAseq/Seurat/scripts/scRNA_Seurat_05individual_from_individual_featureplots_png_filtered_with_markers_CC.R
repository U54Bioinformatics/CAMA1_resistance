...
pbmc <- subset(pbmc, subset = Phase == cellcycle)
table(pbmc$orig.ident)
...
saveRDS(pbmc, file = paste(prefix, "_Seurat_2kgenes_vst_cc.G1.rds", sep=""))
...
print("DE genes by MAST")
print("Before changing Idents")
print(pbmc)
print(table(pbmc$orig.ident))
print(table(pbmc$Marker_genes))
print(table(pbmc$Marker_groups))
print(table(pbmc$Marker_groups1))
print(table(pbmc$Phase))
print(table(Idents(object = pbmc)))
Idents(object = pbmc) <- 'Marker_groups1'
print("After changing Idents")
print(pbmc)
print(table(pbmc$orig.ident))
print(table(pbmc$Marker_genes))
print(table(pbmc$Marker_groups))
...
#marker groups: GV014_mvenus vs. GV013_mvenus
cells.markers <- FindMarkers(object = pbmc, ident.1="GV014_mvenus", ident.2="GV013_mvenus", min.pct = 0, logfc.threshold = 0, assay = 'RNA', slot = 'data', test.use = "MAST")
#output txt file
genes.diff.file <- paste(sample, platform, "top10_genes_GV014mV_vs_GV013mV.gene_diff.txt", sep="_")
genes.id   <- data.table(gene = rownames(cells.markers))
genes.diff <- cbind(genes.id, cells.markers)
...
if (FALSE){
cells.markers <- FindMarkers(object = pbmc, ident.1="GV014_mcherry", ident.2="GV015_mcherry", min.pct = 0, logfc.threshold = 0, assay = 'RNA', slot = 'data', test.use = "MAST")
#output txt file
...
Idents(object = pbmc) <- 'Marker_groups1'
cells.markers <- FindMarkers(object = pbmc, ident.1="GV013_mvenus", ident.2="GV015_mcherry", min.pct = 0, logfc.threshold = 0, assay = 'RNA', slot = 'data', test.use = "MAST")
#output txt file
...
Idents(object = pbmc) <- 'Marker_groups1'
cells.markers <- FindMarkers(object = pbmc, ident.1="GV014_mvenus", ident.2="GV014_mcherry", min.pct = 0, logfc.threshold = 0, assay = 'RNA', slot = 'data', test.use = "MAST")
#output txt file
...
Idents(object = pbmc) <- 'Marker_groups1'
cells.markers <- FindMarkers(object = pbmc, ident.1="GV014_mcherry", ident.2="GV013_mvenus", min.pct = 0, logfc.threshold = 0, assay = 'RNA', slot = 'data', test.use = "MAST")
#output txt file
